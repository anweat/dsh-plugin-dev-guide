...
| 包 | 版本 | 类型 | 说明 | 安装 |
|---|---|---|---|---|
| [dsh-web-search-pro](https://github.com/anweat/dsh-web-search-pro) | 0.1.2 | Host | 增强型、可持久化的网页搜索：多引擎路由（DeepSeek/Exa/DDG/Bing/Jina + GitHub/B站/YouTube/V2EX/小红书/Twitter/Reddit/RSS）、SQLite+LRU 缓存、userscript 风格抽取、Playwright 渲染 | `dsh plugin --profile web add @anweat/dsh-browser dsh-web-search-pro` |
...
- **依赖关系**：dsh-web-search-pro 的浏览器类能力依赖 dsh-browser 提供 `browser` 服务（Cordis `inject` 自动排序）；两个插件都必须作为 profile 的直接依赖安装，避免 pnpm 清理未被直接声明的 bundle
